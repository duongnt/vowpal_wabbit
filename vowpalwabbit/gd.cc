...
inline void specialized_update(vw& all, void* dat, float x, uint32_t fi) {
	...
	float inv_norm = 1.f;
	if (normalized)
		inv_norm /= (w[all.normalized_idx] * s->avg_norm);
	if (adaptive) {
#if defined(__SSE2__) && !defined(VW_LDA_NO_SSE)
	...
	if (!command_example(all, ec)) {
		...
		if (ec->eta_round != 0.) {
			if (all->power_t == 0.5) {
				if (all->adaptive) {
					if (all->normalized_updates)
						generic_train<specialized_update<true, true> >(*all, ec,
								ec->eta_round, true);
					else
						generic_train<specialized_update<true, false> >(*all,
								ec, ec->eta_round, true);
				} else {
					if (all->normalized_updates)
						generic_train<specialized_update<false, true> >(*all,
								ec, ec->eta_round, true);
					else
						generic_train<specialized_update<false, false> >(*all,
								ec, ec->eta_round, true);
				}
			} else
...
void sync_weights(vw& all) {
	...
	for (uint32_t i = 0; i < length && all.reg_mode; i++)
		all.reg.weight_vector[stride * i] = trunc_weight(
				all.reg.weight_vector[stride * i], (float) all.sd->gravity)
				* (float) all.sd->contraction;
	all.sd->gravity = 0.;
...
struct string_value {
	float v;
	string s;
	friend bool operator<(const string_value& first,
...
void audit_feature(vw& all, feature* f, audit_data* a,
		vector<string_value>& results, string prepend, size_t offset = 0) {
	...
	if (a != NULL)
		tempstream << a->space << '^' << a->feature << ':';
			...
			== ((constant * stride * all.weights_per_problem)
					& all.reg.weight_mask))
		tempstream << "Constant:";

	tempstream << (index / (stride * all.weights_per_problem) & all.parse_mask)
			<< ':' << f->x;
	tempstream << ':'
			<< trunc_weight(weights[index], (float) all.sd->gravity)
					* (float) all.sd->contraction;
	if (all.adaptive)
...
void audit_features(vw& all, v_array<feature>& fs, v_array<audit_data>& as,
		vector<string_value>& results, string prepend, size_t offset = 0) {
...
void audit_quad(vw& all, feature& left_feature, audit_data* left_audit,
		v_array<feature> &right_features, v_array<audit_data> &audit_right,
		vector<string_value>& results, uint32_t offset = 0) {
	...
	audit_features(all, right_features, audit_right, results, prepend,
			halfhash + offset);
}

void audit_triple(vw& all, feature& f0, audit_data* f0_audit, feature& f1,
		audit_data* f1_audit, v_array<feature> &right_features,
		v_array<audit_data> &audit_right, vector<string_value>& results,
		uint32_t offset = 0) {
	size_t halfhash = cubic_constant2
			* (cubic_constant * (f0.weight_index + offset) + f1.weight_index
					+ offset);
...
void print_features(vw& all, example* &ec) {
	...
	if (all.lda > 0) {
		...
		for (unsigned char* i = ec->indices.begin; i != ec->indices.end; i++)
			for (audit_data *f = ec->audit_features[*i].begin;
				...
				cout << '\t' << f->space << '^' << f->feature << ':'
						<< (f->weight_index / all.reg.stride & all.parse_mask)
						<< ':' << f->x;
				for (size_t k = 0; k < all.lda; k++)
					cout << ':'
							<< weights[(f->weight_index + k)
									& all.reg.weight_mask];
			}
		...
		for (unsigned char* i = ec->indices.begin; i != ec->indices.end; i++)
			audit_features(all, ec->atomics[*i], ec->audit_features[*i],
					features, empty, ec->ft_offset);
		for (vector<string>::iterator i = all.pairs.begin();
				i != all.pairs.end(); i++) {
			int fst = (*i)[0];
			int snd = (*i)[1];
			for (size_t j = 0; j < ec->atomics[fst].size(); j++) {
				...
				audit_quad(all, ec->atomics[fst][j], a, ec->atomics[snd],
						ec->audit_features[snd], features);
			}
		}

		for (vector<string>::iterator i = all.triples.begin();
				i != all.triples.end(); i++) {
			int fst = (*i)[0];
			int snd = (*i)[1];
			int trd = (*i)[2];
			for (size_t j = 0; j < ec->atomics[fst].size(); j++) {
				...
				for (size_t k = 0; k < ec->atomics[snd].size(); k++) {
					...
					audit_triple(all, ec->atomics[fst][j], a1,
							ec->atomics[snd][k], a2, ec->atomics[trd],
							ec->audit_features[trd], features);
				}
			}
		}
		...
		for (vector<string_value>::iterator sv = features.begin();
				sv != features.end(); sv++)
			cout << '\t' << (*sv).s;
		cout << endl;
	}
...
template<void (*T)(vw&, void*, float, uint32_t)>
float compute_norm(vw& all, example* &ec) { //We must traverse the features in _precisely_ the same order as during training.
	label_data* ld = (label_data*) ec->ld;
	float g = all.loss->getSquareGrad(ec->final_prediction, ld->label)
			* ld->weight;
	if (g == 0)
		return 1.;
...
void local_predict(vw& all, example* ec) {
	...
	ec->final_prediction = finalize_prediction(all,
			ec->partial_prediction * (float) all.sd->contraction);

	if (all.active_simulation) {
		float k = ec->example_t - ld->weight;
		ec->revert_weight = all.loss->getRevertingWeight(all.sd,
				ec->final_prediction, all.eta / powf(k, all.power_t));
	...
	if (ld->label != FLT_MAX) {
		...
		if (all.training && ec->loss > 0.) {
			...
			if (all.adaptive || all.normalized_updates) {
				if (all.power_t == 0.5) {
					if (all.adaptive && all.normalized_updates)
						norm = compute_norm<simple_norm_compute<true, true> >(
								all, ec);
					else if (all.adaptive)
						norm = compute_norm<simple_norm_compute<true, false> >(
								all, ec);
					else
						norm = compute_norm<simple_norm_compute<false, true> >(
								all, ec);
				} else
			...
			if (all.invariant_updates)
				update = all.loss->getUpdate(ec->final_prediction, ld->label,
						eta_t, norm);
			else
				update = all.loss->getUnsafeUpdate(ec->final_prediction,
						ld->label, eta_t, norm);

			ec->eta_round = (float) (update / all.sd->contraction);

			if (all.reg_mode && fabs(ec->eta_round) > 1e-8) {
				double dev1 = all.loss->first_derivative(all.sd,
						ec->final_prediction, ld->label);
				double eta_bar =
						(fabs(dev1) > 1e-8) ? (-ec->eta_round / dev1) : 0.0;

				if (fabs(dev1) > 1e-8)
					all.sd->contraction /=
							(1. + all.l2_lambda * eta_bar * norm);
				all.sd->gravity += eta_bar * sqrt(norm) * all.l1_lambda;
			}
		}
	} else if (all.active)
		ec->revert_weight = all.loss->getRevertingWeight(all.sd,
				ec->final_prediction, all.eta / powf(t, all.power_t));
...
void predict(vw& all, example* ex) {
	...
	if (all.training && all.normalized_updates && ld->label != FLT_MAX
		...
		if (all.power_t == 0.5) {
			...
			if (all.reg_mode % 2)
				prediction = inline_predict<vec_add_trunc_rescale_general>(all,
						ex);
			else
...
void save_load_online_state(vw& all, io_buf& model_file, bool read, bool text) {
	...
	bin_text_read_write_fixed(model_file, (char*) &all.initial_t,
			sizeof(all.initial_t), "", read, buff, text_len, text);
	...
	bin_text_read_write_fixed(model_file, (char*) &all.normalized_sum_norm_x,
			sizeof(all.normalized_sum_norm_x), "", read, buff, text_len, text);
	...
	bin_text_read_write_fixed(model_file, (char*) &all.sd->sum_loss,
			sizeof(all.sd->sum_loss), "", read, buff, text_len, text);

	text_len = sprintf(buff, "weighted_examples %f\n",
			all.sd->weighted_examples);
	bin_text_read_write_fixed(model_file, (char*) &all.sd->weighted_examples,
			sizeof(all.sd->weighted_examples), "", read, buff, text_len, text);
	...
	bin_text_read_write_fixed(model_file, (char*) &all.sd->weighted_labels,
			sizeof(all.sd->weighted_labels), "", read, buff, text_len, text);
	...
	bin_text_read_write_fixed(model_file,
			(char*) &all.sd->weighted_unlabeled_examples,
			sizeof(all.sd->weighted_unlabeled_examples), "", read, buff,
			text_len, text);
	...
	bin_text_read_write_fixed(model_file, (char*) &all.sd->example_number,
			sizeof(all.sd->example_number), "", read, buff, text_len, text);
	...
	bin_text_read_write_fixed(model_file, (char*) &all.sd->total_features,
			sizeof(all.sd->total_features), "", read, buff, text_len, text);
		...
		} else // write binary or text
		{
			...
			if (*v != 0.) {
				...
				if (stride == 2) { //either adaptive or normalized
					...
					brw += bin_text_write_fixed(model_file, (char *) v,
							2 * sizeof(*v), buff, text_len, text);
				} else { //adaptive and normalized
					text_len = sprintf(buff, ":%f %f %f\n", *v, *(v + 1),
							*(v + 2));
					brw += bin_text_write_fixed(model_file, (char *) v,
							3 * sizeof(*v), buff, text_len, text);
				}
			}
		}
		if (!read)
