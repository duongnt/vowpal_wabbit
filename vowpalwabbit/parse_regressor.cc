...
void initialize_regressor(vw& all) {
	...
	all.reg.weight_vector = (weight *) calloc(all.reg.stride * length,
			sizeof(weight));
	if (all.reg.weight_vector == NULL) {
		cerr << all.program_name << ": Failed to allocate weight array with "
				<< all.num_bits << " bits: try decreasing -b <bits>" << endl;
		throw exception();
	}
	if (all.random_weights) {
		for (size_t j = 0; j < length; j++)
			all.reg.weight_vector[j * all.reg.stride] =
					(float) (frand48() - 0.5);
	}
...
void save_load_header(vw& all, io_buf& model_file, bool read, bool text) {
	...
	if (model_file.files.size() > 0) {
		...
		if (read)
			v_length = buf_size;
		bin_text_read_write(model_file, buff2, v_length, "", read, buff,
				text_len, text);
		version_struct v_tmp(buff2);
		if (v_tmp < LAST_COMPATIBLE_VERSION) {
			cout << "Model has possibly incompatible version! "
					<< v_tmp.to_string() << endl;
			throw exception();
		}

		char model = 'm';
		bin_text_read_write_fixed(model_file, &model, 1,
				"file is not a model file", read, "", 0, text);

		text_len = sprintf(buff, "Min label:%f\n", all.sd->min_label);
		bin_text_read_write_fixed(model_file, (char*) &all.sd->min_label,
				sizeof(all.sd->min_label), "", read, buff, text_len, text);

		text_len = sprintf(buff, "Max label:%f\n", all.sd->max_label);
		bin_text_read_write_fixed(model_file, (char*) &all.sd->max_label,
				sizeof(all.sd->max_label), "", read, buff, text_len, text);

		text_len = sprintf(buff, "bits:%d\n", (int) all.num_bits);
		uint32_t local_num_bits = all.num_bits;
		bin_text_read_write_fixed(model_file, (char *) &local_num_bits,
				sizeof(local_num_bits), "", read, buff, text_len, text);
		...
		bin_text_read_write_fixed(model_file, (char *) &pair_len,
				sizeof(pair_len), "", read, buff, text_len, text);
		for (size_t i = 0; i < pair_len; i++) {
			...
			bin_text_read_write_fixed(model_file, pair, 2, "", read, buff,
					text_len, text);
			if (read) {
				string temp(pair, 2);
		...
		bin_text_read_write_fixed(model_file, (char *) &triple_len,
				sizeof(triple_len), "", read, buff, text_len, text);
		for (size_t i = 0; i < triple_len; i++) {
			...
			bin_text_read_write_fixed(model_file, triple, 3, "", read, buff,
					text_len, text);
			if (read) {
		...
		bin_text_read_write_fixed(model_file, (char*) &all.rank,
				sizeof(all.rank), "", read, buff, text_len, text);
		...
		bin_text_read_write_fixed(model_file, (char *) &ngram_len,
				sizeof(ngram_len), "", read, buff, text_len, text);
		for (size_t i = 0; i < ngram_len; i++) {
			...
			bin_text_read_write_fixed(model_file, ngram, 3, "", read, buff,
					text_len, text);
			if (read) {
				string temp(ngram, 3);
		...
		if (read)
			compile_gram(all.ngram_strings, all.ngram, (char*) "grams",
					all.quiet);
		...
		bin_text_read_write_fixed(model_file, (char *) &skip_len,
				sizeof(skip_len), "", read, buff, text_len, text);
		for (size_t i = 0; i < skip_len; i++) {
			...
			bin_text_read_write_fixed(model_file, skip, 3, "", read, buff,
					text_len, text);
			if (read) {
				string temp(skip, 3);
		...
		if (read)
			compile_gram(all.skip_strings, all.skips, (char*) "skips",
					all.quiet);
		bin_text_read_write_fixed(model_file, buff, 0, "", read, "\n", 1, text);
		...
		if (read)
			len = buf_size;
		bin_text_read_write(model_file, buff2, len, "", read, buff, text_len,
				text);
		if (read)
...
void save_predictor(vw& all, string reg_name, size_t current_pass) {
	...
	if (all.save_per_pass)
		sprintf(filename, "%s.%lu", reg_name.c_str(),
				(long unsigned) current_pass);
	else
...
void parse_regressor_args(vw& all, po::variables_map& vm, io_buf& io_temp) {
	if (vm.count("final_regressor")) {
		...
		if (!all.quiet)
			cerr << "final_regressor = " << vm["final_regressor"].as<string>()
					<< endl;
	} else
